...
- **Bubble Tea needs a real TTY.** Popup invocations get one. Direct shell invocations (Bash tool, `cc-rich --pane %X | head`) don't — the binary will print `could not open a new TTY: open /dev/tty: device not configured`. Test the popup with `tmux display-popup -E "cc-rich --pane #{pane_id} 2>/tmp/log; sleep 5"` to capture stderr.
